      ...
      let options =data.specialtyId? {
        ...
        include: [
          ...
          },
          {
            ...
            attributes: ["specialtyId", "provinceId", "addressClinic", "packagesId"],
            include: [
              {
                model: db.Specialty,
                as: "specialtyData",
    ...
    try {
      ...
      if (checkObj.isValid === false) {
        ...
        if (doctorInfor) {
          ...
          // doctorInfor.packagesIdId = inputData.packagesId;
          ...
          await db.Doctor_Infor.create({
            ...
            // packagesId: inputData.packagesId,
          });
        }
        resolve({
          errCode: 0,
    ...
    try {
      if (!doctorId || !date) {
        ...
        let data = await db.Booking.findAll({
          where: { statusId: ["S2", "S3", "S4"], doctorId: doctorId, date: date},
          include: [
            {
              model: db.User,
              as: "patientData",
    ...
    try {
      if (!data.doctorId || !data.patientId || !data.timeType) {
        ...
        await db.History.create({
          patientId: data.patientId,
          doctorId: data.doctorId,
          reason: data.patientReason ? data.patientReason : "",
          statuscf: "✔", // Thêm trường statuscf
          createdAt: new Date(),
          updatedAt: new Date(),
        });
...
//       if (data.specialtyId) {
//         options.include[2].where.specialtyId = data.specialtyId;
//       }
//       if (data.firstName) {
//         options.where.firstName = {
//           [Op.like]: "%" + data.firstName + "%",
//         };
//       }
//       if (data.lastName) {
//         options.where.lastName = {
...
//       }
//       if (data.position) {
//         options.where.positionId = data.position;
//       }
