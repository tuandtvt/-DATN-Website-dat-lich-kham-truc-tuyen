...
//   return new Promise(async (resolve, reject) => {
//     try {
//       if (!data.token || (!data.doctorId && !data.packagesId)) {
//         resolve({
//           errCode: 1,
//           errMessage: "Missing required parameter",
//         });
...
//           appointment = await db.Booking.findOne({
//             where: { packagesId: data.packagesId, token: data.token, statusId: "S1" },
//             raw: false,
//           });
//         }
...
// //             },
// //             order: [['createdAt', 'DESC']],
// //             attributes: ["id", "patientId", "doctorId", "reason", "createdAt"],
// //             include: [
// //               {
// //                 model: db.User,
// //                 as: "doctorDataHistory",
...
// //             },
// //             order: [['createdAt', 'DESC']],
// //             attributes: ["id", "patientId", "doctorId", "reason", "createdAt"], // Include 'reason' in the attributes
// //             include: [
// //               {
// //                 model: db.User,
// //                 as: "doctorDataHistory",
...
//             },
//             order: [['createdAt', 'DESC']],
//             attributes: ["id", "patientId", "doctorId", "reason", "createdAt"], // Bao gồm 'reason' trong danh sách thuộc tính
//             include: [
//               {
//                 model: db.User,
//                 as: "doctorDataHistory",
...
// Hàm xây dựng URL để xác nhận email
let buildUrlEmail = (doctorId, token) => {
    let result = `${process.env.URL_REACT}/verify-booking?token=${token}&doctorId=${doctorId}`;
    return result;
};
    ...
    return new Promise(async (resolve, reject) => {
        try {
            ...
            if (!data.email || !data.doctorId || !data.timeType || !data.date || !data.patientName) {
                ...
                // Kiểm tra giới hạn đặt lịch trên bảng Schedule
                let schedule = await db.Schedule.findOne({
                    where: { 
                        date: data.date,
                        timeType: data.timeType,
                ...
                if (schedule) {
                    if (schedule.currentNumber < schedule.maxNumber) {
                        // Tăng số lượng đặt lịch hiện tại và lưu lại
                        schedule.currentNumber = parseInt(schedule.currentNumber) + 1;
                        await schedule.save();

                        // Tạo token để xác nhận lịch hẹn qua email
                        let token = uuidv4();
                        ...
                        // Tạo hoặc tìm kiếm thông tin bệnh nhân
                        let user = await db.User.findOne({
                            where: { email: data.email },
                        });

                        // Tạo bản ghi lịch hẹn trong bảng Booking
                        if (user) {
                            await db.Booking.create({
                                statusId: "S1",
                                doctorId: data.doctorId,
                        ...
                        resolve({
                            errCode: 0,
                            errMessage: "Save information of patient succeed!",
                        });
                    } else {
                        // Trả về thông báo lỗi nếu đã vượt quá giới hạn đặt lịch
                        resolve({
                            errCode: 3,
                            errMessage: "Limit max number booking!",
                        });
                    ...
                    // Trả về thông báo lỗi nếu không tìm thấy lịch làm việc của bác sĩ
                    resolve({
                        errCode: 3,
                        errMessage: "Limit max number booking!",
                    });
    ...
    return new Promise(async (resolve, reject) => {
        try {
            if (!data.token || (!data.doctorId && !data.packagesId)) {
                ...
                if (appointment) {
                    // Cập nhật trạng thái lịch hẹn sang "đã xác nhận"
                    appointment.statusId = "S2";
                    await appointment.save();
                    resolve({
                        errCode: 0,
