  ...
  return new Promise(async (resolve, reject) => {
    try {
      let apointmentDones = await db.Booking.findAll({
        where: { statusId: "S3" },
        attributes: ["id", "createdAt", "statusId", "patientId"],
        raw: true,
        nest: true,
      });
      // console.log(apointmentDones)
...
let getTotalHealthAppointmentDone1 = () => {
  return new Promise(async (resolve, reject) => {
    try {
      let apointmentDones = await db.Booking.findAll({
        where: { statusId: "S3" },
      ...
      resolve({
        errCode: 0,
        data: { totalHealthApointmentDone: totalHealthApointmentDone,
                apointmentDones:apointmentDones
        },
      });
    } catch (e) {
      reject(e);
...
module.exports = {
  ...
  getTotalHealthAppointmentDone1: getTotalHealthAppointmentDone1,
