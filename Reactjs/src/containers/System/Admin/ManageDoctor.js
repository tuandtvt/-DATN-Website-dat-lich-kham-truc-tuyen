...
//   render() {
//     let { hasOldData, listSpecialty, listDoctorsRaw, firstName,lastName,position,specialtyId} = this.state;
//     let positions = this.state.positionArr;
//     let specialty = this.state.specialtyArr;
...
//                     <th scope="col"><FormattedMessage id="admin.manage-doctor.addressClinic" /></th>
...
//                         <td>{doctor.Doctor_Infor.addressClinic}</td>
...
//           <div className="col-3">
//             <label>
//               <FormattedMessage id="manage-user.specialty" />
//             </label>
//             <select
//               className="form-control"
//               onChange={(event) => {
//                 this.onChangeInput(event, "specialtyId");
//               }}
//               value={specialtyId}
//             >
//               <option value="">
...
import "react-markdown-editor-lite/lib/index.css";
import Select from "react-select";
import { getDetailInforDoctor, getAllSpecialty } from "../../../services/userService";
import { filterDoctors } from "../../../services/doctorService"; // Đảm bảo bạn đã nhập đúng các dịch vụ
import { withRouter } from '../../../utils/withRouter';
...
class ManageDoctor extends Component {
  constructor(props) {
    ...
    this.state = {
      ...
      specialtyId: "",
      positionArr: [],
      specialtyArr: [], // Thêm biến này để lưu danh sách chuyên khoa
      contentMarkdown: "",
      contentHTML: "",
      selectedOption: "",
      description: "",
  ...
  componentDidMount = async () => {
    ...
    // Lấy danh sách chuyên khoa từ API và cập nhật vào state
    let specialtyResponse = await getAllSpecialty();
    if (specialtyResponse && specialtyResponse.errCode === 0) {
      this.setState({
        specialtyArr: specialtyResponse.data // Cập nhật state với dữ liệu chuyên khoa
      });
    }

    // Kiểm tra dữ liệu chuyên khoa đã cập nhật chưa
    console.log('Specialty Array:', this.state.specialtyArr);
  }
  ...
  render() {
    let { hasOldData, specialtyArr, listDoctorsRaw, firstName, lastName, position, specialtyId } = this.state;
    let positions = this.state.positionArr;
    let { language } = this.props;
    ...
    return (
      <div>
        ...
        <div className="row">
          ...
          <div className="col-3">
            <label>
              <FormattedMessage id="manage-user.speciality" />
            </label>
            <select
              className="form-control"
              onChange={(event) => {
