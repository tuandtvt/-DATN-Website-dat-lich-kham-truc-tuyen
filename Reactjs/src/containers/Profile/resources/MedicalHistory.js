...
//                                       {/* <div class="pointer text-primary" onClick={()=>history.push(`/detail-package/${item.doctorDataHistory.Doctor_Infor.packageId}`)}>{item.doctorDataHistory.Doctor_Infor.packgeData.name}</div> */}
//                                   </td>
//                                   <td>
  ...
  return (
    ...
    >
      <div>
        ...
        <div className="row">
          <div className="col-12">
            <table className="table table-hover mt-45">
              <thead>
                <tr>
                  <th scope="col">#</th>
                  <th scope="col">
              ...
              <tbody>
                {histories.map((item, index) => {
                  let drugs = item.drugs ? JSON.parse(item.drugs) : [];
                  return (
                    <tr key={index}>
                      <th scope="row">{index + 1}</th>
                      <td>{moment(item.createdAt).format("DD/MM/YYYY")}</td>
