...
import { connect } from "react-redux";
import { push } from "connected-react-router";
import * as actions from "../../store/actions"; // Thay đổi đường dẫn đến actions của bạn nếu cần
import "./Signup.scss";
import { FormattedMessage } from "react-intl";
  ...
  checkValidateInput = () => {
    ...
    for (let i = 0; i < arrInput.length; i++) {
      if (!this.state[arrInput[i]]) {
        ...
        toast.error(
          this.props.language === "en"
            ? `Missing parameter: ${arrInput[i]}`
            : `Chưa nhập đủ thông tin: ${arrInput[i]}`
        );
        break;
      }
    }
